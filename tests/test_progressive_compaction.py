...
async def test_tool_result_truncation_phase1():
    ...
    # Four results make the first 25% wave nonempty. Only its large result
    # is unprotected, and truncating it alone is enough to reach the target.
    context = SimpleContextManager(
        ...
        # Isolate the compaction phase from the notice reserve.
    ...
    # Removal is not a substitute for the truncation this test promises to check.
    assert tool_result is not None, "The truncated result must remain in the view"
    ...
    assert [
        ...
        if message.get("tool_call_id", "").startswith("recent-call-")
    ] == ["small result"] * 3
